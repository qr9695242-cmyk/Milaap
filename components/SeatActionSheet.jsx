...
import { sendSeatRequest } from "@/lib/roomRequests";
...
export default function SeatActionSheet({
 ...
 // Locked, empty seat and not the host: send the host a request instead
 // of just being stuck with "no actions available for this seat".
 if (!occupied && seat.locked && !isHost && !isMySeat && hostUid) {
 options.push({
 label: "Request Seat",
 onClick: () =>
 run(() =>
