...
export const GAME_STAKES = [...MILAAP_ECONOMY.gameStakes];
export const MATCH_STAKES = GAME_STAKES.filter(Boolean);
...
/** Settles one session exactly once. Rewards are . */
...
export function scoreToReward(score, { entryCoins = 0, multiplier = 2, cap = 5000 } = {}) {
 ...
 const entry = clampInt(entryCoins);
 if (entry > 0) return entry + Math.min(cap, Math.floor(s * multiplier));
