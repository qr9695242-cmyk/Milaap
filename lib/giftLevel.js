...
const MIN_GIFT_COINS = 1000000; // level 1 threshold — tune this to move the whole ladder
const MAX_GIFT_COINS = 2000000000; // level 200 threshold (kept at the same 2000x span as before)
...
// Geometric growth from MIN_GIFT_COINS (level 1) to MAX_GIFT_COINS (level 200),
// rounded to a clean number so thresholds don't look like random noise.
const GROWTH_RATE = Math.pow(MAX_GIFT_COINS / MIN_GIFT_COINS, 1 / (MAX_GIFT_LEVEL - 1));
function coinsForLevel(level) {
  if (level <= 0) return 0;
  const raw = MIN_GIFT_COINS * Math.pow(GROWTH_RATE, level - 1);
  const step =
    raw < 100000 ? 1000 :
    raw < 1000000 ? 10000 :
    raw < 10000000 ? 100000 :
    raw < 100000000 ? 1000000 :
    10000000;
