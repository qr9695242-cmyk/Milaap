...
export function familyLevelForDiamonds(totalDiamonds = 0) {
 if (totalDiamonds >= 100000) return 5;
 if (totalDiamonds >= 25000) return 4;
 if (totalDiamonds >= 10000) return 3;
 if (totalDiamonds >= 2000) return 2;
 return 1;
}

export async function createFamily({ name, leaderId, leaderName, leaderAvatar = "" }) {
 const ref = await addDoc(collection(db, "families"), {
 name,
 leaderId,
 leaderName,
 members: [{ uid: leaderId, name: leaderName, avatar: leaderAvatar }],
 totalDiamonds: 0,
 level: 1,
...
export async function joinFamily(familyId, uid, name, avatar = "") {
