...
export const FRAME_CATALOG = [
  ...
  // frame_denim's art has a smaller transparent window than the other frames,
  // so it needs a bigger overlay (see FramedAvatar) or the photo pokes out past it.
  { id: "frame_denim", name: "Denim Loop", priceCoins: 4000, rarity: "common", emoji: "🔵", image: "/frames/frame_denim.png", gradient: "linear-gradient(135deg,#4B6C9E,#2C3E5C)", frameScale: 2.24 },
 ...
 * and saves the resulting URL to Firestore
 * (`decorationMedia/{type}_{itemId}`). Firestore here is just a small
 * pointer doc, not the file store — the actual media lives in Cloudinary
 * under milaap/decorations/{type}/. Anyone opening the shop page picks this up
 * live via listenDecorationMedia and it overrides the item's built-in
