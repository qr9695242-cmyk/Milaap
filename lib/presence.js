...
import { notifyAllUsersOnline } from "./notifications";

const HEARTBEAT_MS = 30_000;
export const ONLINE_THRESHOLD_MS = 45_000;

export function startPresenceHeartbeat(uid, name) {
  if (!uid) return () => {};
  ...
  // Let everyone know this user just came online — fired ONCE here (app
  // open/login), never from the interval below or the tab-focus handler,
  // otherwise every active user would get spammed every 30 seconds.
  notifyAllUsersOnline({ uid, name }).catch(() => {});
