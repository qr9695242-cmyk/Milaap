...
// IMPORTANT: coins, match state, dice and diamond settlement are never
...
function diamondsFor(pot, count) { return Math.floor(Number(pot || 0) / (count === 4 ? LUDO_4_RATE : LUDO_2_RATE)); }
...
exports.ludoCreate = onCall(async (request) => {
  ...
  await db.runTransaction(async (tx) => {
    ...
    tx.create(ref, {
      ...
      winner: null, diamondsWon: null, createdAt: new Date(), serverVersion: 1,
    });
  });
...
exports.ludoRoll = onCall(async (request) => {
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (movable.length === 1) {
      ...
      if (out.won) {
        const diamondsWon = diamondsFor(data.pot, countFor(data.playerCount));
        const winnerRef = db.collection("users").doc(uid);
        const winner = await tx.get(winnerRef);
        tx.update(winnerRef, { diamonds: Number(winner.data().diamonds || 0) + diamondsWon });
        tx.update(ref, { tokensByColor: out.tokensByColor, diceValue: dice, selectableIds: [], canRoll: false, status: "finished", winner: uid, diamondsWon, settledAt: new Date(), message: "Winner!" });
        result = { dice, autoMoved: true, finished: true, diamondsWon };
        return;
      }
...
exports.ludoMove = onCall(async (request) => {
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (out.won) {
      ...
      const winner = await tx.get(winnerRef);
      tx.update(winnerRef, { diamonds: Number(winner.data().diamonds || 0) + diamondsWon });
      tx.update(ref, { tokensByColor: out.tokensByColor, selectableIds: [], canRoll: false, status: "finished", winner: uid, diamondsWon, settledAt: new Date(), message: "Winner!" });
      result = { finished: true, diamondsWon };
      return;
    }
    const six = dice === 6 ? Number(data.sixStreak || 0) + 1 : 0;
    const keep = dice === 6 && six < 3;
