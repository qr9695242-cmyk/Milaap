     ...
     <div className="flex items-start justify-between gap-3">
       <div className="min-w-0">
         ...
         <div className="mt-2 space-y-1 text-xs text-mist">
           <p><span className="font-bold text-ink/80">Name:</span> {w.name || "—"}</p>
           <p><span className="font-bold text-ink/80">Method:</span> {w.method || "—"}</p>
           <p><span className="font-bold text-ink/80">Phone / Account:</span> {w.account || "—"}</p>
           <p><span className="font-bold text-ink/80">Payout:</span> Rs {Number(w.cashValue || 0).toLocaleString()}</p>
           <p><span className="font-bold text-ink/80">Rate:</span> Rs {Number(w.rupeesPerDiamond || 0).toLocaleString()} / Diamond</p>
         </div>
         <p className="mt-2 text-[10px] text-mist">UID: {w.uid}</p>
       </div>
       <span className="rounded-full bg-gold/10 px-2 py-1 text-[10px] font-bold text-gold">PENDING</span>
     </div>
     <div className="mt-3 grid grid-cols-2 gap-2">
       <button disabled={busyId === w.id} onClick={() => handleWithdrawalStatus(w.id, "approved")} className="rounded-xl bg-emerald-400/15 py-2 text-xs font-bold text-emerald-300 ring-1 ring-emerald-300/20">Mark Paid / Approve</button>
