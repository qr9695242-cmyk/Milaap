...
import { listenIncomingSeatRequests, respondSeatRequest } from "@/lib/roomRequests";
...
export default function AudioRoomPage() {
  ...
  const speakerOnRef = useRef(true); // mirrors speakerOn for use inside the Agora event handler closure
  ...
  // Host-only: incoming "Request Seat" taps from viewers on locked seats
  // (components/SeatActionSheet.jsx → lib/roomRequests.js).
  useEffect(() => {
    if (!isHost || !user) return;
    const unsub = listenIncomingSeatRequests(String(roomId), user.uid, setSeatRequests);
    return () => unsub();
  }, [isHost, roomId, user?.uid]);

  async function handleAcceptSeatRequest(req) {
    await respondSeatRequest(String(roomId), req.id, "accepted");
    try {
      await takeSeat(String(roomId), req.seatIndex, req.fromUid, req.fromName);
    } catch (e) {
      // Seat may have been taken/relocked in the meantime — request is
      // already marked accepted either way, nothing more to do.
    }
  }

  async function handleRejectSeatRequest(req) {
    await respondSeatRequest(String(roomId), req.id, "rejected");
  }
  ...
  function toggleSpeaker() {
    setSpeakerOn((prev) => {
      ...
      Object.values(remoteAudioRef.current).forEach((track) => {
  ...
  async function handleQuickChatSend(text) {
    await sendChatMessage(String(roomId), {
      uid: user.uid,
      name: profile?.displayName || "User",
      text,
    });
    ...
    async function join() {
      try {
        ...
        client.on("user-published", async (remoteUser, mediaType) => {
          ...
          remoteUser.audioTrack.setVolume?.(speakerOnRef.current ? 100 : 0);
  ...
  // Live "X ms" indicator (screenshot reference) — real round-trip time
  // from the Agora client we're already connected with, not a fake number.
  useEffect(() => {
  ...
  async function handleSeatTap(seat) {
    ...
    if (seat.uid === user.uid) {
      ...
      // Locked + empty: open the sheet so they can send the host a
      // "Request Seat" (see SeatActionSheet + lib/roomRequests.js)
    ...
    >
      <header className="flex items-center justify-between px-4 py-3 pt-safe">
        <div className="flex min-w-0 items-center gap-3">
          <div className="min-w-0">
            ...
            <div className="mt-0.5 flex flex-wrap items-center gap-2 text-[10px] text-mist">
              ...
              <span className="rounded-full bg-black/30 px-2 py-0.5 font-semibold text-gold">
                🏆 {sessionCoins.toLocaleString()}
              </span>
              {pingMs != null && (
                <span
                  className={`rounded-full bg-black/30 px-2 py-0.5 font-semibold ${
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <Link
            href="/messages"
            aria-label="Messages"
            className="flex h-9 w-9 items-center justify-center rounded-full bg-black/30 text-sm text-ink ring-1 ring-white/15 active:scale-95"
          >
            ✉️
          </Link>
          <RoomMoreMenu />
      ...
      {isHost && (
        <RoomRequests
          ...
          onAccept={handleAcceptSeatRequest}
      ...
      {showWelcome && (
        <div className="relative mx-4 mt-2 rounded-xl bg-black/55 p-2.5 text-[10px] leading-relaxed text-ink/85 ring-1 ring-white/10">
          <button
      ...
      {showGiftPanel && <div ref={giftBarRef} className="mt-2">
        <GiftBar
          roomId={String(roomId)}
          fromUid={user.uid}
      ...
      {!isHost && !confirmExit && (
        <button
          ...
          aria-label="Room Match — find a new room"
          className="fixed bottom-24 right-4 z-40 flex flex-col items-center gap-1 rounded-full bg-teal-400 px-3 py-2.5 text-black shadow-glow active:scale-95"
        >
          <span className="text-lg leading-none">🔀</span>
          <span className="text-[8px] font-bold leading-none">Match</span>
        </button>
      )}

      <div className="mt-4 flex-1 overflow-hidden border-t border-white/5">
        <LiveChat roomId={String(roomId)} uid={user.uid} name={profile?.displayName || "User"} hideInput />
      </div>
      ...
      <RoomBottomBar
        ...
        onOpenGifts={() => { setShowGiftPanel(true); requestAnimationFrame(() => giftBarRef.current?.scrollIntoView({ behavior: "smooth", block: "center" })); }}
        unread={unreadCount}
      />
    </main>
  );
