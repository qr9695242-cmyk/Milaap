...
const emptyTokens = (colors = ["red", "yellow"]) => createInitialTokens(colors);
...
export default function LudoPage() {
  ...
  const [localColors, setLocalColors] = useState(["red", "yellow"]);
  ...
  async function startMatch(matchMode) {
    ...
    } catch (e) { setError(friendlyFirebaseError(e, "Ludo server se match create nahi hua. Firebase Cloud Functions deploy hain ya nahi check karein.")); }
  ...
  async function joinWaiting(id) {
    ...
    } catch (e) { setError(friendlyFirebaseError(e, "Ludo server se join nahi hua. Firebase Cloud Functions aur login status check karein.")); }
    finally { setBusy(false); }
  }
  ...
  async function joinCode() {
    ...
    catch (e) { setError(friendlyFirebaseError(e, "Ludo room join nahi hua. Room code aur Firebase service check karein.")); }
    finally { setBusy(false); }
  }

  function localNextTurn() {
    const next = (localTurnIdx + 1) % localColors.length; setLocalTurnIdx(next); setLocalDice(null); setLocalSelectable([]); setLocalSix(0);
    setLocalMessage(`${COLOR_META[localColors[next]].label} ka turn — dice roll karein!`);
  }
  function localFinishMove(dice) {
    if (dice === 6 && localSix + 1 < 3) { setLocalSix((s) => s + 1); setLocalDice(null); setLocalMessage(`${COLOR_META[localColors[localTurnIdx]].label} ko dobara turn mila.`); }
    else localNextTurn();
  }
  function localMove(tokenId, dice) {
    const color = localColors[localTurnIdx];
  ...
  function localRoll() {
    ...
    setTimeout(() => { const value = rollDice(); setLocalDice(value); setRolling(false); const color = localColors[localTurnIdx]; const movable = getMovableTokens(localTokens, color, value);
  ...
  async function onlineRoll() {
    ...
    try {
      ...
      setError(friendlyFirebaseError(e, "Dice roll server ne accept nahi kiya. Firebase Ludo service check karein."));
    } finally {
      setRolling(false);
    }
  }
  async function onlineMove(tokenId) {
    ...
    try {
      ...
      setError(friendlyFirebaseError(e, "Token move server ne accept nahi ki. Match state check karein."));
    }
  }

  const displayTokens = phase === "local" || phase === "localFinished" ? localTokens : (match?.tokensByColor || emptyTokens());
  const activeColors = phase === "local" || phase === "localFinished" ? localColors : (match?.activeColors || ["red", "yellow"]);
    ...
    <main className="game-screen min-h-screen bg-void text-ink flex items-center justify-center px-5">
      <div className="w-full max-w-sm rounded-3xl bg-panel p-7 text-center ring-1 ring-white/10">
        ...
        <div className="mt-6 rounded-2xl bg-panel2 p-4 text-left text-sm">
          ...
          <div className="mt-2 flex justify-between"><span className="text-mist">Winner Coins</span><b className="text-gold">🪙 {ludoWinnerCoins(match?.pot || stake * playerCount).toLocaleString()}</b></div>
        </div>
  ...
  if (phase === "found") {
    ...
    const winnerCoins = ludoWinnerCoins(match?.pot || stake * count);
    return (
      <main className="game-screen min-h-screen bg-void text-ink flex items-center justify-center px-5">
        <div className="w-full max-w-sm rounded-3xl bg-panel p-6 text-center ring-1 ring-white/10">
          ...
          <div className="mt-6 rounded-2xl bg-panel2 p-4 text-left text-sm">
            ...
            <div className="mt-2 flex justify-between"><span className="text-mist">Winner Gets</span><b className="text-gold">🪙 {winnerCoins.toLocaleString()}</b></div>
            <p className="mt-2 text-[11px] text-mist">Coin-to-coin — winner takes the whole pot.</p>
          </div>
