...
export default function LudoPage() {
 ...
 const hasAnyLegalMove = activeTokens.some((p) => canMove(p, dice));
 if (!hasAnyLegalMove) {
   const nextTurn = dice === 6 ? state.turnIndex : (state.turnIndex + 1) % state.playerCount;
   const skipped = { ...state, dice: 1, lastRoll: dice, diceRolled: false, turnIndex: nextTurn };
   setState(skipped);
   setMessage(dice === 6 ? `6 aya, lekin koi legal move nahi. ${state.players?.[nextTurn]?.name || "Player"} dobara roll kare.` : `${state.players?.[nextTurn]?.name || "Next player"} ki turn.`);
   if (online && matchId) {
