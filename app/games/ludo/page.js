...
function canMove(position, dice) {
 const rollValue = Number(dice);
 if (!Number.isInteger(rollValue) || rollValue < 1 || rollValue > 6) return false;
 if (position === FINISH) return false;
 if (position === -1) return rollValue === 6;
 return position + rollValue <= FINISH;
}

function hasLegalMove(tokens, dice) {
 return (tokens || []).some((position) => canMove(position, dice));
}
...
export default function LudoPage() {
 ...
 setRolling(false);
 setMessage(result === 6 ? "6 aya! Ab chamakti goti par tap karein — goti bahar aa jayegi." : `Dice: ${result}. Apni legal goti par tap karein.`);
 }
 }, 75);
 }

 async function moveToken(color, index) {
 if (!state || state.status !== "playing" || !myTurn || !state.diceRolled) return;

 const activePlayer = state.players?.[state.turnIndex];
 const activeColor = activePlayer?.color;
 if (activeColor && color !== activeColor) {
 setMessage(`Ab ${activePlayer?.name || activeColor.toUpperCase()} ki turn hai.`);
 return;
 }

 const dice = Number(state.dice || state.lastRoll || 0);
 const activeTokens = state.tokens?.[color] || [];
 if (!hasLegalMove(activeTokens, dice)) {
   const nextTurn = dice === 6 ? state.turnIndex : (state.turnIndex + 1) % state.playerCount;
   const skipped = { ...state, dice: 0, lastRoll: dice, diceRolled: false, turnIndex: nextTurn };
   setState(skipped);
   setMessage(dice === 6
     ? `6 aya, lekin koi legal move nahi. ${state.players?.[nextTurn]?.name || "Player"} dobara roll kare.`
     : `${state.players?.[nextTurn]?.name || "Next player"} ki turn.`);
   if (online && matchId) {
     try { await updateCasualMatch(matchId, skipped); }
     catch (e) { setError(e.message || "Turn sync nahi hua."); }
   }
   return;
 }

 const current = activeTokens[index] ?? -1;
 if (!canMove(current, dice)) {
 setMessage(dice === 6 ? "6 par ye goti bahar nahi aa sakti — doosri goti select karein." : "Is goti ko is roll par move nahi kar sakte.");
 return;
 }
 ...
 // Cut opponent tokens unless the destination is a safe cell.
 ...
 const keepTurn = dice === 6 && !won;
 const nextTurn = keepTurn ? state.turnIndex : (state.turnIndex + 1) % state.playerCount;
 ...
 winner: won ? activePlayer?.uid : null,
 turnIndex: nextTurn,
 };
 ...
 if (won) {
 setPhase("finished");
 setMessage(`${activePlayer?.name || color.toUpperCase()} player won!`);
 if (online && matchId) {
 try { await updateCasualMatch(matchId, updated); }
 catch (e) { setError(e.message || "Winner save nahi hua."); }
 }
 return;
 }

 if (keepTurn) {
 setMessage(`6! ${activePlayer?.name || "Player"} dobara roll karein.`);
 } else {
 const nextPlayer = updated.players[updated.turnIndex];
 setMessage(`${nextPlayer?.name || "Next player"} ki turn.`);
 }
 ...
 <p className="mt-3 text-[10px] text-white/50">6 par goti bahar • peeli chamakti goti selectable hai • ★ safe cell • 6 par extra turn</p>
...
function LudoBoard({ state, onToken }) {
 ...
 className={`absolute z-20 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-white shadow-[0_3px_7px_rgba(0,0,0,.35)] transition-transform ${
...
function HomeTokens({ color, state, onToken }) {
 ...
 className={`absolute z-20 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-white shadow-lg transition-transform ${
 state.diceRolled && state.players?.[state.turnIndex]?.color === color && canMove(position, state.dice)
 ? "animate-pulse scale-110 ring-4 ring-yellow-300/90"
 : ""
 }`}
