          ...
          return (
            ...
            >
              ...
              <div className="relative flex h-16 w-16 items-center justify-center">
                {/* Circular avatar swatch — same shape as the real profile photo */}
                <div
                  className="flex h-16 w-16 items-center justify-center overflow-hidden rounded-full text-2xl ring-1 ring-white/10"
                  style={{ background: item.free ? "rgba(255,255,255,0.05)" : item.gradient }}
                >
                  {item.free && "🚫"}
                </div>
                {/* Frame ring overlaid full-size around the circle, same scale logic as FramedAvatar,
                    so the shop preview matches exactly how it looks once equipped. */}
                {item.video ? (
                  <video
                    src={item.video}
                    className="pointer-events-none absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 select-none"
                    style={{ width: 64 * (item.frameScale || 1.82), height: 64 * (item.frameScale || 1.82), objectFit: "contain" }}
                    autoPlay
                    loop
                    muted
                    playsInline
                  ...
                  !item.free && (
                    <span className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 text-2xl">{item.emoji}</span>
                  )
                )}
              </div>
