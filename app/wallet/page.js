...
export default function WalletPage() {
  ...
  // default so there's no flash of "0 coins" before Firestore responds.
  const [exchangeRate, setExchangeRateState] = useState(DIAMOND_TO_COIN_RATE);
    ...
    <main className="min-h-screen bg-void pb-28">
      ...
      <PremiumCard className="mx-5 mt-4 p-5">
        ...
        <p className="mt-1 text-[11px] text-mist">
          1 diamond = {diamondsToCoins(1, exchangeRate)} coins · Minimum {MIN_EXCHANGE_DIAMONDS} diamonds
        </p>
        ...
        {exchangeAmount && !isNaN(parseInt(exchangeAmount, 10)) && (
          <p className="mt-2 text-[11px] text-mist">
            = {diamondsToCoins(parseInt(exchangeAmount, 10) || 0, exchangeRate)} coins
          </p>
        )}
